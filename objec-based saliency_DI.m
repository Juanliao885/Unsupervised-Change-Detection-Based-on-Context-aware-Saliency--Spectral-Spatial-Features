...
L=imread('CubbieStation_50.tif');L=L+1;%分割结果
img_d=sqrt((double(I2(:,:,1))-double(I1(:,:,1))).^2+(double(I2(:,:,2))-double(I1(:,:,2))).^2+(double(I2(:,:,3))-double(I1(:,:,3))).^2);
ymax=255;ymin=0;
xmax = max(max(img_d)); %求得InImg中的最大值
xmin = min(min(img_d)); %求得InImg中的最小值
out_img_d= round((ymax-ymin)*(img_d-xmin)/(xmax-xmin) + ymin); %归一化并取整
...
%E:\document\liaojuan\change detection\20190909\参考
%Mimg=zeros(X*Y,Z);
...
[edges_x edges_y] = find(tmp==1); seg_edges = [edges_x edges_y];%seg_edges有问题，没有找完
%% K邻域
L1=seg_edges;
for i=1:nseg
